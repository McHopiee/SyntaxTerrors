...
const cookie = {
  ...
  updateDisplay() {
    if (cookieCountDisplay){
      cookieCountDisplay.innerHTML = this.cookies.toString();
    }
  },
  fetchStoredCookies() {
...
const shop = {
  /**
   * @type {Array<{name: string, emoji: string, price: number, itemEffected?: string, multiplier?: number}>}
   */
  upgrades: [],
  tab: "shop",
  /**
   * @type {Array<{name: string, emoji: string, price: number, originalPrice: number, cookiesPerSecond?: number, multiplier?: number}>}
   */
  forSale: [],
  updateShopDisplay() {
    if(shopContainer) {
...
if (cookieButton) {
  ...
  console.error("cookieButton is not found in the DOM.");
}